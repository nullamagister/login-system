...
var WebView_1 = __importDefault(require("../../view/WebView"));
var Server_1 = __importDefault(require("../Server"));
var router = express_1.default.Router();
var webView;
setImmediate(function () { return webView = new WebView_1.default(Server_1.default.getDB()); });
...
router.get('/home', function (req, res) {
    ...
    if (req.session.user) {
        webView.getData(req.session.user).then(function (_res) {
            var user = _res.data.user;
            var online = _res.data.activeUsers;
            var offline = _res.data.inActiveUsers;
            console.log('Render: home.pug');
            res.render('home', { user: user, online: online, offline: offline });
        }, function (err) {
...
router.post("/register", function (req, res) {
    ...
    if (req.session.user) {
    ...
    else if (!['male', 'female'].includes(req.body.gender.toLocaleLowerCase())) {
        console.log('Render: error.pug');
        ...
        webView.register(newUser).then(function () {
...
router.post("/login", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    ...
    return __generator(this, function (_a) {
        ...
        if (req.session.user) {
            ...
            password = req.body.password;
            webView.login(username_1, password).then(function () {
...
router.get("/logout", function (req, res) {
    ...
    if (req.session.user) {
        webView.logout(req.session.user).then(function (sysRes) {
...
router.put("/update", function (req, res) {
    ...
    if (req.session.user && req.body.newUser) {
        webView.update(req.session.user, JSON.parse(req.body.newUser)).then(function () {
...
router.delete("/delete", function (req, res) {
    ...
    if (req.session.user) {
        webView.delete(req.session.user).then(function () {
            req.session.destroy(function (err) { return err ? (res.render("error", { message: 'Internal Error' })) : ''; });
            console.log("Json: {redirect: '/'}");
            res.json({ redirect: '/' }).end();
        }, function () {
